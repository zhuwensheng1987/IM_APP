...
+ (id) movieViewControllerWithDelegate:(id <KxMovieCallback>)delegate
{
    id<KxAudioManager> audioManager = [KxAudioManager audioManager];
    [audioManager activateAudioSession];
    return [[KxMovieViewController  alloc] initWithDelegate:delegate];
}

- (id) initWithDelegate:(id <KxMovieCallback>)delegate
{
    self = [super initWithNibName:nil bundle:nil];
    if (self) {
        if ([[[UIDevice currentDevice] systemVersion] floatValue] >= 7)
...
{
    [self pause];
    [self playMovieImp:path pos:pos parameters:parameters];
}
...
- (void)loadView
{
    ...
    _preButton.frame = CGRectMake(width * 0.5 - 110, 5, 40, 40);
    _preButton.backgroundColor = [UIColor clearColor];
    _preButton.showsTouchWhenHighlighted = YES;
    [_preButton setImage:[UIImage imageNamed:@"kxmovie.bundle/playback_prev"] forState:UIControlStateNormal];
    [_preButton addTarget:self action:@selector(prevDidTouch:) forControlEvents:UIControlEventTouchUpInside];

    _nextButton = [UIButton buttonWithType:UIButtonTypeCustom];
    _nextButton.frame = CGRectMake(width * 0.5 + 70, 5, 40, 40);
    _nextButton.backgroundColor = [UIColor clearColor];
    _nextButton.showsTouchWhenHighlighted = YES;
    [_nextButton setImage:[UIImage imageNamed:@"kxmovie.bundle/playback_next"] forState:UIControlStateNormal];
    [_nextButton addTarget:self action:@selector(nextDidTouch:) forControlEvents:UIControlEventTouchUpInside];
    ...
    [_bottomHUD addSubview:_preButton];
    [_bottomHUD addSubview:_nextButton];
...
- (void) nextDidTouch:(id) sender
{
    if (self.playing)
        [self pause];
    if ( self.delegate )
        [self.delegate onNext:self curPos:_moviePosition];
    NSLog(@"next movie");
}

- (void) prevDidTouch:(id) sender
{
    if (self.playing)
        [self pause];
    if ( self.delegate )
        [self.delegate onPre:self curPos:_moviePosition];
    NSLog(@"pre movie");
}
...
- (void) updatePlayButton
{
    ...
    {
        _nextButton.hidden = ![self.delegate hasNext];
        _preButton.hidden = ![self.delegate hasPre];
    }
}
