...
NSString * const FFMovieParameterMaxBufferedDuration = @"FFMovieParameterMaxBufferedDuration";
NSString * const FFMovieParameterDisableDeinterlacing = @"FFMovieParameterDisableDeinterlacing";
...
@interface FFMovieViewController () <ALMoviePlayerInterface>
{
...
@implementation FFMovieViewController
...
+ (id) movieViewControllerWithDelegate:(id <FFMovieCallback>)delegate
{
    ...
    return [[FFMovieViewController alloc] initWithDelegate:delegate];
}
...
-(void) playMovieImp:(NSString *)path pos:(CGFloat)pos parameters: (NSDictionary *) parameters
{
    ...
    decoder.interruptCallback = ^BOOL(){
        __strong FFMovieViewController *strongSelf = weakSelf;
...
- (void) viewDidAppear:(BOOL)animated
{
    ...
    if (_decoder) {
        ...
        [_controls showLoadingIndicators];
    }
               ...
               withError: (NSError *) error
{
    ...
    if (!error && decoder) {
        ...
        if (_parameters.count) {
            ...
            val = [_parameters valueForKey: FFMovieParameterMinBufferedDuration];
            if ([val isKindOfClass:[NSNumber class]])
                _minBufferedDuration = [val floatValue];
            
            val = [_parameters valueForKey: FFMovieParameterMaxBufferedDuration];
            if ([val isKindOfClass:[NSNumber class]])
                _maxBufferedDuration = [val floatValue];
            
            val = [_parameters valueForKey: FFMovieParameterDisableDeinterlacing];
            if ([val isKindOfClass:[NSNumber class]])
        ...
        if (self.isViewLoaded) {
            ...
            if ( [_controls isLoadingIndicators] ) {
                [_controls hideLoadingIndicators];
                // if (self.view.window)
                [self restorePlay];
            }
...
- (void) setupPresentView
{
    ...
    [_controls setDurationSliderMaxMinValues:_decoder.duration];
...
{
    return _decoder ? _decoder.duration : 0.f;
}
...
{
    [self setMoviePosition: _moviePosition + f];
}
