...
- (id)initWithMoviePlayer:(id<ALMoviePlayerInterface>)moviePlayer style:(ALMoviePlayerControlsStyle)style {
    ...
    if (self) {
        ...
        _barHeight = [FFHelper iOSVersion] >= 7.0 ? 70.f : 50.f;
...
- (void)setup {
    ...
    if (_style == ALMoviePlayerControlsStyleFullscreen || (_style == ALMoviePlayerControlsStyleDefault && [_moviePlayer isFullscreen])) {
        ...
        _doneButton = [[ALButton alloc] init];
        [_doneButton setTitle:@"Done" forState:UIControlStateNormal];
        [_doneButton setTitleShadowColor:[UIColor blackColor] forState:UIControlStateNormal];
        _doneButton.titleLabel.shadowOffset = CGSizeMake(1.f, 1.f);
        [_doneButton.titleLabel setFont:[UIFont systemFontOfSize:14.f]];
    ...
    [_playPauseButton setSelected:[_moviePlayer isPlaying] ? NO : YES];
...
- (void)durationSliderValueChanged:(UISlider *)slider {
    ...
    double totalTime = floor([self.moviePlayer getDuration]);
...
- (void)playPausePressed:(UIButton *)button {
    [self.moviePlayer isPlaying] ? [self.moviePlayer onPause] : [self.moviePlayer onPlay];
    [self performSelector:@selector(hideControls:) withObject:nil afterDelay:self.fadeDelay];
}

- (void)scalePressed:(UIButton *)button {
    
    if (_style == ALMoviePlayerControlsStyleEmbedded || (_style == ALMoviePlayerControlsStyleDefault && ![_moviePlayer isFullscreen])) {
        ...
        if ([self.moviePlayer getCurrentPlaybackRate] != 1.f) {
            [self.moviePlayer setCurrentPlaybackRate:1.f];
        }
...
-(void) updateMoviePlayback:(CGFloat)currentTime total:(CGFloat)totalTime
{
    [self setTimeLabelValues:currentTime totalTime:totalTime];
...
- (void)layoutSubviews {
    ...
    if ( [FFHelper isIpad] )
    {
        ...
        {
            CGFloat t = mainViewBounds.origin.x; mainViewBounds.origin.x = mainViewBounds.origin.y; mainViewBounds.origin.y = t;
            t = mainViewBounds.size.width; mainViewBounds.size.width = mainViewBounds.size.height; mainViewBounds.size.height = t;
        }
    }
    ...
    CGFloat paddingBetweenPlaybackButtons = mainViewBounds.size.width <= iPhoneScreenPortraitWidth ? 20.f : 30.f;
    CGFloat paddingBetweenLabelsAndSlider = 10.f;
    CGFloat sliderHeight = 34.f; //default height
    CGFloat volumeHeight = 20.f;
    CGFloat volumeWidth = [FFHelper isIpad] ? 210.f : 120.f;
    ...
    if (self.style == ALMoviePlayerControlsStyleFullscreen || (self.style == ALMoviePlayerControlsStyleDefault && self.moviePlayer.isFullscreen)) {
        ...
        CGRect rect = CGRectMake(0, 0, mainViewBounds.size.width, self.barHeight);
        ...
        self.bottomBar.frame = rectBottom;
        ...
        self.bottomBar.frame = CGRectMake(0, mainViewBounds.size.height - self.barHeight, mainViewBounds.size.width, self.barHeight);
        ...
        self.doneButton.frame = CGRectMake(self.bottomBar.frame.size.width - paddingFromBezel - fullscreenWidth, self.barHeight/2 - fullscreenHeight/2, fullscreenWidth, fullscreenHeight);
        self.airplayView.frame = CGRectMake(self.doneButton.frame.origin.x - paddingBetweenButtons - airplayWidth, self.barHeight/2 - airplayHeight/2, airplayWidth, airplayHeight);
